...
export function validateStep2BasicInfo(
  ...
  if ((seoTitle?.trim().length ?? 0) > 70) {
    errors.push({
      field: 'seoTitle',
      message: 'El titulo SEO no puede superar los 70 caracteres',
    })
  }

  if ((seoDescription?.trim().length ?? 0) > 160) {
    errors.push({
      field: 'seoDescription',
      message: 'La descripcion SEO no puede superar los 160 caracteres',
    })
  }
 ...
 * @returns Validation result with any errors
 */
export function validateStep4Category(categoryId: string, subCategory: string): ValidationResult {
  const errors: ValidationError[] = []
 ...
 * @returns Validation result with any errors
 */
export function validateStep5SizesColors(sizes: string[], colors: string[]): ValidationResult {
  const errors: ValidationError[] = []
 ...
 * @returns Validation result with any errors
 */
export function validateWizardStep(stepNumber: number, productData: ProductData): ValidationResult {
  switch (stepNumber) {
    case 1:
      return validateStep1Photos(productData.images)
    case 2:
      return validateStep2BasicInfo(
        productData.name,
